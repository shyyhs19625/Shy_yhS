/**
 * ========= 青龙 =========
 * 变量格式：export dingchao=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 */
...
!(async () => {
	...
	if (!(await Envs()))  	
		...
		for (let index = 0; index < dingchaoArr.length; index++) {
 ...
 function signin(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `https://apiv2.dingstock.net/xserver/user/fortune/checkin`,
			headers: {
				"Accept": "*/*",
				...
				"app": "iOS2.10.0(2022040302)",
				"version": "2.10.0",
				"User-Agent": "mobile/2.10.0 (cool.dingstock.mobile; build:2022040302; iOS 15.3.1) Alamofire/5.5.0",
         "token": data,
			},
		}
...
async function Envs() {
	if (dingchao) {
		if (dingchao.indexOf("@") != -1) {
			dingchao.split("@").forEach((item) => {
				dingchaoArr.push(item);
			});
		} else {
			dingchaoArr.push(dingchao);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 dingchao`)
		return;
